...
```mermaid
flowchart LR
    accTitle: Search parameter migration
    accDescr: Updated string, numeric, and enum parameters use an object containing a value array and optional comparison condition. Updated date parameters use an object containing a date property. Sending an old literal value for an updated parameter produces an HTTP 406 deserialization error.
    input["Updated search parameter"] --> type{"Parameter type"}
    type -->|"String, numeric, or enum"| extended["Object with value array<br/>and optional compareCondition"]
    type -->|"Date, DateTime, or DateTimeUTC"| date["Object with date property"]
    input -->|"Legacy literal value"| error["HTTP 406<br/>deserialization error"]
    extended --> accepted["Typed API search model"]
    date --> accepted
```
